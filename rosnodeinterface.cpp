...
// Generated 17-Feb-2022 10:54:53
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"accel_4th_order\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(accel_4th_order_M, (NULL));
        accel_4th_order_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(accel_4th_order_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(accel_4th_order_M)
                == (NULL)) || rtmGetStopRequested(accel_4th_order_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(accel_4th_order_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
